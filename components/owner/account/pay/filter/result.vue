...
import Item from '@/components/owner/account/pay/filter/Item'
import OrderSheet from '@/components/owner/account/pay/sheet/OrderSheet'
import { accountAll } from '@/api/account/pay'
...
export default {
    ...
    checkAllItem: function(newVal, oldVal) {
      if (newVal) {
        ...
        this.ids = this.list.map(item => {
          return item.payableBillUkid
        })
        this.money = this.list.map(item => {
          return {
            payableBillUkid: item.payableBillUkid,
  ...
  methods: {
    ...
    checkOut(value) {
      if (value.check) {
        this.money.push({
          payableBillUkid: value.data.payableBillUkid,
          remainingAmount: value.data.remainingAmount
        })
      } else {
        this.money.forEach((ele, index) => {
          if (ele.payableBillUkid === value.data.payableBillUkid) {
