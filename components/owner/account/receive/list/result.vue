...
import Item from '@/components/owner/account/receive/list/Item'
import { fetchList } from '@/api/account/receive'
...
export default {
  ...
  watch: {
    list: function(newVal, oldVal) {
      this.filters = newVal.filter(item => {
        return (
          item.receivableBillStatus === 10 || item.receivableBillStatus === 50
        )
      })
    },
    params: {
  ...
  methods: {
    ...
    goFilter() {
      this.$router.push({
        name: 'owner-account-receive-filter',
