...
export default {
  ...
  computed: {
    showStartDate() {
      return this.$format(this.filters.createTimeBegin, DateStyle)
    },
    showEndDate() {
      return this.$format(this.filters.createTimeEnd, DateStyle)
    },
    hasStartDate() {
      return !!this.filters.createTimeBegin
    },
    hasEndDate() {
      return !!this.filters.createTimeEnd
    }
  },
  ...
  methods: {
    openStartPicker() {
      ...
      if (this.hasStartDate) {
        this.pickerDate = new Date(this.filters.createTimeBegin)
      }
      if (this.hasEndDate) {
        this.maxDate = new Date(this.filters.createTimeEnd)
      }
    ...
    openEndPicker(type, currentDate) {
      ...
      if (this.hasEndDate) {
        this.pickerDate = new Date(this.filters.createTimeEnd)
      }
      if (this.hasStartDate) {
        this.minDate = new Date(this.filters.createTimeBegin)
      }
      this.$refs.picker.open()
    },
