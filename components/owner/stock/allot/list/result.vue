  ...
  <div>
    ...
    >
      <template v-for="(item, index) in list">
        <item :key="'item' + index" :data="item" @cancel="onItemCancel(index)"/>
...
import Item from '@/components/owner/stock/allot/list/Item'
import { getAllotList } from '@/api/owner/stock'
...
export default {
  ...
  methods: {
    async fetch(append = true) {
      let { success, data } = await getAllotList({
    ...
    onItemCancel(cancelIndex) {
      ...
      let nItem = Object.assign(oItem, {
        allocationStatusName: '已取消',
        allocationStatus: 990
      })
      this.list = replaceBy(this.list, nItem, 'allocationUkid')
    }
  }
}
</script>
