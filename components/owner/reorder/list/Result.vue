  ...
  <div>
    ...
    >
      <template v-for="(item, index) in list">
        <Item :key="'item_' + item.returnUkid" :data="item" @cancel="onItemCancel(index)"/>
        <wms-whitespace :key="'sep_' + item.returnUkid" size="10" bgcolor="rgba(0,0,0,.15)"/>
      </template>
    </div>
...
import { getList } from '@/api/owner/reorder'
...
export default {
  ...
  methods: {
    ...
    onItemCancel(cancelIndex) {
      ...
      this.list = replaceBy(this.list, nItem, 'returnUkid')
    }
  }
}
</script>
