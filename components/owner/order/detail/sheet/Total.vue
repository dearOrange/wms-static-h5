<template>
  <div class="total">
    ...
    <div v-if="isConfirm" class="item">
      ...
      <span class="value">{{ shippedCount }}</span>
    </div>
    <div class="item">
...
export default {
  ...
  computed: {
    ...
    shippedCount() {
      return sumBy(this.data.ordDetailList, item => item.shippedQty)
    },
    amountCount() {
      return sumBy(this.data.ordDetailList, item => {
        return (
          item.orderQty * item.unitPrice * this.data.baOwnerCustomer.discount
        )
      })
    }
  }
